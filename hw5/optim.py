...
def rmsprop(theta, dtheta, config=None):
  ...
  #############################################################################
  lr, dr, epl, cache = config['learning_rate'], config['decay_rate'], config['epsilon'], config['cache']
  cache = dr * cache + (1 - dr) * (dtheta ** 2)
  next_theta = theta - lr * dtheta / (np.sqrt(cache) + epl)
  config['cache'] = cache
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def adam(theta, dtheta, config=None):
  ...
  # stored in config.                                                         #
  #############################################################################
  lr, beta1, beta2, epl = config['learning_rate'], config['beta1'], config['beta2'], config['epsilon']
  ...
  mb = m / (1 - beta1 ** t)
  vb = v / (1 - beta2 ** t)
  next_theta = theta - lr * mb / (np.sqrt(vb) + epl)
