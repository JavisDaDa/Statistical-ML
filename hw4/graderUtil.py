...
class Part:
    def __init__(self, name, gradeFunc, maxPoints, maxSeconds):
        ...
        if maxSeconds != None and not isinstance(maxSeconds, int): raise Exception(
            "Invalid maxSeconds: %s" % maxSeconds)
        # Specification
        self.name = name
...
class Grader:
    ...
    def grade(self):
        ...
        for part in parts:
            ...
            print('----- END PART %s [took %s, %s/%s points]' % (
            part.name, endTime - startTime, part.points, part.maxPoints))
